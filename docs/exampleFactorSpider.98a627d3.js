...
(function () {
  ...
  */
  ...
  function init() {
    id("submit_btn").addEventListener('click', handleAddCountry); // window.addEventListener('scroll', handleScroll);
    ...
    var targetNode = document.getElementById('selected_country_list'); // Options for the observer (which mutations to observe)
  ...
  function handleAddCountry() {
    ...
    drawGraph();
    qsa(".delete_btn").forEach(function (deleteButton) {
      return deleteButton.addEventListener('click', handleRemoveCountry);
    });
  }
  ...
  function drawGraph() {
    var goalCountries = _toConsumableArray(window.selectedCountryList);
    ...
    goalCountries.sort();
    d3.csv(_spiderCountryFactor.default).then(function (data) {
      var allCountryData = [];
      var i = 0;
      data.forEach(function (d) {
        ...
        if (d.Country === goalCountries[i]) {
          var countryAxes = [{
            axis: 'Economy (GDP per Capital)',
            value: d["Economy (GDP per Capita)"]
          }, {
            axis: 'Family or social support',
            value: d["Family or social support"]
          }, {
            axis: 'Freedom to make life choices',
            value: d["Freedom to make life choices"]
          }, {
            axis: 'Generosity',
            value: d["Generosity"]
          }, {
            axis: 'Health (Life Expectancy)',
            value: d["Health (Life Expectancy)"]
          }, {
            axis: 'Trust (Government Corruption)',
            value: d["Family or social support"]
          }];
          var countryData = {
            name: d.Country,
      ...
      if (goalCountries.length != 1) {
        for (var _i = 0; _i < goalCountries.length - 1; _i++) {
  ...
  var RadarChart = function RadarChart(parent_selector, data, options) {
    ...
    var parent = d3.select(parent_selector); //Remove whatever chart with the same id/class was present before

    parent.select("svg").remove(); //Initiate the radar chart SVG

    var svg = parent.append("svg").attr("width", cfg.w + cfg.margin.left + cfg.margin.right).attr("height", cfg.h + cfg.margin.top + cfg.margin.bottom).attr("class", "radar"); //Append a g element
    ...
    }).attr("class", "line").style("stroke", "#CDCDCD").style("stroke-width", "2px"); //Append the labels at each axis
    ...
    var radarLine0 = d3.radialLine().curve(d3.curveLinearClosed).radius(function (d) {
      return rScale(0);
    }).angle(function (d, i) {
      return i * angleSlice;
    });
    ...
    blobWrapper.append("path").attr("class", "radarArea").attr("d", function (d) {
      ...
      //Dim all blobs
      parent.selectAll(".radarArea").transition().duration(200).style("fill-opacity", 0.1); //Bring back the hovered over blob
    ...
    blobWrapper.selectAll(".radarCircle").data(function (d) {
      return d.axes;
    }).enter().append("circle").attr("class", "radarCircle").attr("r", 0).attr("cx", 0).attr("cy", 0).transition().duration(1000).attr("r", cfg.dotRadius).attr("cx", function (d, i) {
    ...
    if (cfg.legend !== false && _typeof(cfg.legend) === "object") {
      ...
      if (cfg.legend.title) {
        var title = legendZone.append("text").attr("class", "title").attr('transform', "translate(".concat(cfg.legend.translateX, ",").concat(cfg.legend.translateY, ")")).attr("x", cfg.w - 70).attr("y", 10).attr("font-size", "12px").attr("fill", "#404040").text(cfg.legend.title);
      }

      var legend = legendZone.append("g").attr("class", "legend").attr("height", 100).attr("width", 200).attr('transform', "translate(".concat(cfg.legend.translateX, ",").concat(cfg.legend.translateY + 20, ")")); // Create rectangles markers
      ...
      legend.selectAll('text').data(names).enter().append("text").attr("x", cfg.w - 52).attr("y", function (d, i) {
        ...
        this.setAttribute("font-weight", "normal");
        d3.select(this).style("background-color", "transparent").style("fill", "black").style("opacity", "1");
      });
    }
...
},{"d3":"../node_modules/d3/index.js","../static/spiderCountryFactor.csv":"../static/spiderCountryFactor.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","exampleFactorSpider.js"], null)
//# sourceMappingURL=/exampleFactorSpider.98a627d3.js.map