...
(function () {
  ...
  */
  ...
  var allData = []; //console.log("factorGraph");
  ...
  function drawGraph() {
    ...
    var allData = [];
    selectedCountry = window.selectedCountry; //console.log("draw graph", selectedCountry);

    line_svg = d3.select('#factor_graph').append('svg').attr("id", "line-chart").attr("width", w + margin.left + margin.right + 130).attr("height", h + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    line_svg.append("g").attr("transform", "translate(0," + h + ")").attr("class", "myXaxis");
    line_svg.append("g").attr("class", "myYaxis"); //console.log("drawGraph window.selectedCountry", window.selectedCountry);
    //console.log("readFactorGraphData");

    Promise.all([(0, _d.csv)(_FactorLineChart.default)]).then(function (data) {
      data[0].forEach(function (d) {
        //console.log("readFactorGraphData", d);
        allData.push(addData(d.Country, parseInt(d.Year), parseFloat(d["Economy (GDP per Capita)"]), parseFloat(d["Family or social support"]), parseFloat(d["Health (Life Expectancy)"]), parseFloat(d["Freedom to make life choices"]), parseFloat(d["Trust (Government Corruption)"]), parseFloat(d["Generosity"])));
      });
      ...
      line_svg.append("text").attr("class", "factor_title").text(selectedCountry + " Factor Score by Years").attr("fill", "black").attr("x", w / 2).attr("y", 0 - margin.top / 2).attr("text-anchor", "middle").style("font-size", "13px"); //console.log("allDataddddd", allData);
      //console.log("aaaa", [allData[0]]);

      var currentCountry = allData.filter(function (e) {
      ...
      fc_circle = line_svg.append("circle").style("r", 3).attr("fill", "grey").attr("class", "guide_circle");
      factors.forEach(function (d) {
        var currentFactor = current.filter(function (e) {
          return e.Factor === d;
        ...
        var test = line_svg.append("path").datum(currentFactor).attr("class", "lines").attr('d', line).style("stroke-width", 3).style("fill", "none").attr("stroke", myColor.find(function (el) {
          return el.Factor === d;
        })["Color"]).attr("stroke-linejoin", "round").attr("opacity", 0.7).attr("stroke-linecap", "round").on('mouseover', function (d, i) {
          ...
          var s = d[index]["Score"]; //console.log("Year: " + y + " Factor: " + f + " Score: " + s);

          line_svg.selectAll(".lines").transition().duration(50).attr("opacity", "0.2");
          d3.select(this).transition().duration('50').attr('opacity', '.85');
          line_svg.append('text').attr("id", "factor_score").text("Year: " + yr + " Factor: " + f + " Score: " + Math.round(s * 100) / 100).attr("fill", "black").attr("x", w / 2).attr("y", -20).attr("text-anchor", "middle").style("font-size", "10px");
          fc_circle.style("opacity", 0.8).attr("cx", x(yr)).attr("cy", y(s)).attr("fill", myColor.find(function (el) {
            return el.Factor === f;
          })["Color"]);
          ...
          fc_circle.style("opacity", 0);
        });
        var pathLength = test.node().getTotalLength(); // D3 provides lots of transition options, have a play around here:
        // https://github.com/d3/d3-transition

        var transitionPath = d3.transition().ease(d3.easeSin).duration(900);
        test.attr("stroke-dashoffset", pathLength).attr("stroke-dasharray", pathLength).transition(transitionPath).attr("stroke-dashoffset", 0);
      });

      var _loop = function _loop(i) {
        line_svg.append("rect").attr("class", "legends").attr("x", w + 7).attr("y", -60 + 20 * i).attr("width", 10).attr("height", 10).style("fill", myColor.find(function (el) {
          return el.Factor === factors[i];
        })["Color"]);
...
},{"d3":"../node_modules/d3/index.js","../static/FactorLineChart.csv":"../static/FactorLineChart.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","factorGraph.js"], null)
//# sourceMappingURL=/factorGraph.1b691d92.js.map