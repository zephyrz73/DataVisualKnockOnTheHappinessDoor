...
},{"../clip/rectangle.js":"jRQA","../identity.js":"NNW9","../transform.js":"pfKH","./fit.js":"Ycma","../math.js":"JQgf"}],"Y5z7":[function(require,module,exports) {
"use strict";
...
},{"./dist/package.js":"pT13","d3-array":"K0bd","d3-axis":"mp0m","d3-brush":"tkh5","d3-chord":"Iy8J","d3-collection":"S3hn","d3-color":"Peej","d3-contour":"SiBy","d3-dispatch":"D3zY","d3-drag":"kkdU","d3-dsv":"EC2w","d3-ease":"pJ11","d3-fetch":"grWT","d3-force":"oYRE","d3-format":"VuZR","d3-geo":"Ah6W","d3-hierarchy":"Kps6","d3-interpolate":"k9aH","d3-path":"OTyq","d3-polygon":"H15P","d3-quadtree":"lUbg","d3-random":"Gz2j","d3-scale":"zL2z","d3-scale-chromatic":"ado2","d3-selection":"ysDv","d3-shape":"maww","d3-time":"hQYG","d3-time-format":"UYpZ","d3-timer":"rdzS","d3-transition":"UqVV","d3-voronoi":"rLIC","d3-zoom":"MHdZ"}],"O9ZS":[function(require,module,exports) {
module.exports = "https://cse442-20f.github.io/FP-Knock-On-The-Happiness-Door/country_no_duplicate.5a5b0950.csv";
},{}],"LyQK":[function(require,module,exports) {
module.exports = "https://cse442-20f.github.io/FP-Knock-On-The-Happiness-Door/2015_2019_no_duplicate.85481c91.csv";
},{}],"Focm":[function(require,module,exports) {
"use strict";

var _country_no_duplicate = _interopRequireDefault(require("../static/country_no_duplicate.csv"));

var _2019_no_duplicate = _interopRequireDefault(require("../static/2015_2019_no_duplicate.csv"));
...
(function () {
  ...
  */
  ...
  function init() {
    ...
    var country_list = [];
    Promise.all([d3.csv(_country_no_duplicate.default), d3.csv(_2019_no_duplicate.default)]).then(function (data) {
      ...
      line_svg = d3.select('#years_chart').append('svg').attr("id", "line-chart").attr("width", w + margin.left + margin.right + 170).attr("height", h + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
      line_svg.append("g").attr("transform", "translate(0," + h + ")").attr("class", "myXaxis");
      line_svg.append("g").attr("class", "myYaxis").attr("transform", "translate(" + -12 + ",  0 )");
  ...
  function handleFactorSelect() {
    if (!window.selectedFactorList.includes(document.getElementById("factor_select").value) && window.selectedFactorList.length < 3) {
  ...
  function removeFactorElement() {
    ...
    var index = selectedFactorList.indexOf(firstChildValue);
  ...
  function handleCountrySubmit() {
    if (!window.selectedCountryList.includes(document.getElementById("myInput").value) && window.selectedCountryList.length <= 5) {
  ...
  function DrawFocus() {
    ...
    var tooltip = line_svg.append("text").attr("class", "tooltips").attr("id", "tooltip").style("font-size", "12px").attr("fill", "grey").attr("stroke", "white").attr("stroke-width", 0.05).attr("font-weight", "bold").style('display', 'none').attr("text-anchor", "middle").attr("dy", "0.35em");
    var tooltip_for_rank = line_svg.append("text").attr("class", "tooltips").attr("id", "tooltipR").style("font-size", "12px").attr("fill", "grey").attr("stroke", "white").attr("stroke-width", 0.05).attr("font-weight", "bold") // line_svg.append("div").attr("class", "tooltips").attr("id","tooltip")
    // .attr("opacity", 1)
    ...
    // .attr("padding", "10px")
    .style('display', 'none').attr("text-anchor", "middle").attr("dy", "0.35em");
    focusline = line_svg.append("line").attr("class", "focus_line").attr("x1", w).attr("x2", w).attr("y1", -20).attr("y2", h).attr("opacity", 0).attr("stroke-width", 2).attr("stroke", "pink").attr("stroke-dasharray", "8,2").style("pointer-events", "none");

    for (var i = 0; i < selectedCountryList2.length; i++) {
      var ftext = line_svg.append("text").attr("class", "focus_text").attr("fill", window.myColor.find(function (el) {
        return el.Country === selectedCountryList2[i];
      })["Color"]).attr("id", "focustext" + i).attr("font-size", 13 + "px").attr("x", w + 5).attr("y", y(HappinessScore[(i + 1) * 5 - 1]["Happiness Score"])).attr("fill-opacity", 0).text(Math.round(HappinessScore[(i + 1) * 5 - 1]["Happiness Score"] * 100) / 100);
    }

    rectForPointer = line_svg.append('rect').attr("class", "rect_pointer").style("fill", "none").style("pointer-events", "all");

    for (var i = 0; i < selectedCountryList2.length; i++) {
      line_svg.append("circle").attr("class", "focus_circle").attr("id", "focuscircle" + i) // .attr("border-style", "solid")
      // .attr("border-width", "3px")
      // .attr("border-color", myColor.find(el => el.Country === selectedCountryList2[i])["Color"])
      .attr("fill", window.myColor.find(function (el) {
        return el.Country === selectedCountryList2[i];
      })["Color"]).attr("cx", w).attr("cy", y(HappinessScore[(i + 1) * 5 - 1]["Happiness Score"])).attr("opacity", 0).attr("r", 3).style("pointer-events", "all").on("mouseover", function (d, i) {
        ...
        tooltip_point.attr("cx", this.cx.baseVal.value).attr("cy", this.cy.baseVal.value).attr("opacity", 1);
        ...
        tooltip.attr('x', this.cx.baseVal.value + 10).attr('y', this.cy.baseVal.value - 30).transition().style('display', 'block').text(selectedCountryList2[currID] + ": " + Math.round(HappinessScore[(currID + 1) * 5 - 1 + (selectedData.Year - 2019)]["Happiness Score"] * 100) / 100);
        var happyRank;

        if (currID == 0) {
          happyRank = "N/A";
        } else if (selectedData.Year == 2015) {
          happyRank = allAboutCountry_2015.find(function (d) {
            return d.Country === selectedCountryList2[currID];
          })["Happiness Rank"];
        } else if (selectedData.Year == 2016) {
          happyRank = allAboutCountry_2016.find(function (d) {
            return d.Country === selectedCountryList2[currID];
          })["Happiness Rank"];
        } else if (selectedData.Year == 2017) {
          happyRank = allAboutCountry_2017.find(function (d) {
            return d.Country === selectedCountryList2[currID];
          })["Happiness Rank"];
        } else if (selectedData.Year == 2018) {
          happyRank = allAboutCountry_2018.find(function (d) {
            return d.Country === selectedCountryList2[currID];
          })["Happiness Rank"];
        } else if (selectedData.Year == 2019) {
          happyRank = allAboutCountry_2019.find(function (d) {
            return d.Country === selectedCountryList2[currID];
          })["Happiness Rank"];
        }

        tooltip_for_rank.attr('x', this.cx.baseVal.value + 10).attr('y', this.cy.baseVal.value - 15).transition().style('display', 'block').text("World Rank: " + happyRank); // tooltip
        ...
        tooltip_for_rank.transition().style('display', 'none').text(""); //tooltip.attr("opacity", 0);

        line_svg.selectAll(".lines").transition().duration(200).attr("opacity", 0.8);
      });
    }

    rectForPointer.attr('width', w).attr('height', h).on('mouseover', mouseover).on('mousemove', mousemove).on('mouseout', mouseout);
  }

  function mouseover() {
    focusline.transition("fadeFocus").duration(400).attr("opacity", 1);

    for (var count = 0; count < selectedCountryList2.length; count++) {
      line_svg.selectAll("#focustext" + count).transition("fadeFocus").duration(700).attr("fill-opacity", 1);
      line_svg.selectAll("#focuscircle" + count).transition("fadeFocus").duration(700).attr("opacity", 1);
    }
  }
  ...
  function mousemove() {
    ...
    if (typeof d0 != 'undefined' && typeof d1 != 'undefined') {
      selectedData = x0 - d0.Year < d1.Year - x0 ? d1 : d0;
      line_svg.selectAll(".focus_line").transition().duration(700).attr("x1", x(selectedData.Year)).attr("x2", x(selectedData.Year));

      for (var count = 0; count < selectedCountryList2.length; count++) {
        line_svg.selectAll("#focustext" + count).transition().duration(700).attr("x", x(selectedData.Year) + 5).attr("y", y(HappinessScore[(count + 1) * 5 - 1 + (selectedData.Year - 2019)]["Happiness Score"]) - 7).text(Math.round(HappinessScore[(count + 1) * 5 - 1 + (selectedData.Year - 2019)]["Happiness Score"] * 100) / 100);
        line_svg.selectAll("#focuscircle" + count).transition().duration(700).attr("cx", x(selectedData.Year)).attr("cy", y(HappinessScore[(count + 1) * 5 - 1 + (selectedData.Year - 2019)]["Happiness Score"]));
      }
    }
  }

  function mouseout() {
    focusline.transition().duration(400).attr("opacity", 0);

    for (var count = 0; count < selectedCountryList2.length; count++) {
      line_svg.selectAll("#focustext" + count).transition("fadeFocus").duration(700).attr("fill-opacity", 0);
      line_svg.selectAll("#focuscircle" + count).transition("fadeFocus").duration(700).attr("opacity", 0);
    }
  }

  function drawList() {
    var table_svg = d3.select("#table").append("svg").attr("height", 1).attr("width", 1);
    var table = d3.select("#table").append("table").attr("id", "rank-table").attr("class", "table table-condensed table-striped");
    var thead = table.append("thead");
    var tbody = table.append("tbody");
    var headUse = ["Rank", "Country/Region", "Happiness Score"];
    thead.append("tr");
    thead.select("tr").append("th").text("Rank");
    thead.select("tr").append("th").text("Country/Region").style("text-align", "center");
    thead.select("tr").append("th").text("Happiness Score");

    for (var num = 0; num < 15; num++) {
      var temp = tbody.append("tr");
      temp.append("td").text(num + 1).style("text-align", "center").property("value");
      temp.append("td").style("text-align", "center").text(allAboutCountry_2019[num].Country);
      temp.append("td").style("text-align", "center").text(allAboutCountry_2019[num]["Happiness Score"]);
    }
    ...
    tbody.selectAll("tr").on("click", function (d) {
      ...
      d3.select(this).style("background-color", "pink").style("opacity", "0.8").style("color", "#f5fbff").style("cursor", "pointer");
    }).on("mouseout", function (d) {
      d3.select(this).style("background-color", "transparent").style("color", "#883658").style("opacity", "1");
    }); // .on("click",function(d){
  ...
  function drawYearsPlotChart(line_svg) {
    ...
    window.selectedCountryList.forEach(function (d) {
      ...
      var temp2015 = allAboutCountry_2015.find(function (el) {
        return el.Country === d;
      });
      var temp2016 = allAboutCountry_2016.find(function (el) {
        return el.Country === d;
      });
      var temp2017 = allAboutCountry_2017.find(function (el) {
        return el.Country === d;
      });
      var temp2018 = allAboutCountry_2018.find(function (el) {
        return el.Country === d;
      });
      var temp2019 = allAboutCountry_2019.find(function (el) {
        return el.Country === d;
      });
    ...
    x.domain(d3.extent(HappinessScore, function (d) {
      return d.Year;
    }));
    line_svg.selectAll(".myXaxis").call(xAxis);
    y.domain([d3.min(HappinessScore, function (d) {
      return +d["Happiness Score"] - 0.4;
    }), d3.max(HappinessScore, function (d) {
      return +d["Happiness Score"] + 0.2;
    })]);
    line_svg.selectAll(".myYaxis").transition().duration(1000).call(yAxis);
    line_svg.append("text").attr("class", "ylabel").attr("text-anchor", "end").attr("y", -60).attr("x", -h + 170).attr("font-size", 12 + "px").attr("dy", ".75em").attr("transform", "rotate(-90)").text("Happiness Score (out of 10)").attr("fill", "rgb(83, 69, 69)");
    var newTemp = 0;
    selectedCountryList2.forEach(function (d) {
      var currentCountry = HappinessScore.filter(function (e) {
        return e.Country === d;
      });
      ...
      var path = line_svg.append("path").datum(currentCountry).attr("class", "lines").attr("id", function (d) {
        return "line" + newTemp;
      }).attr('d', line).style("stroke-width", 2.5).style("fill", "none").attr("stroke", window.myColor.find(function (el) {
        return el.Country === d;
      })["Color"]).attr("stroke-linejoin", "round").attr("opacity", 0.8).attr("stroke-linecap", "round");
      newTemp++;
      var pathLength = path.node().getTotalLength(); // D3 provides lots of transition options, have a play around here:
    ...
    var _loop = function _loop(_i) {
      line_svg.append("rect").attr("class", "legends").attr("x", w + 50).attr("y", 30 + 25 * _i).attr("width", 10).attr("height", 10).style("fill", window.myColor.find(function (el) {
        return el.Country === selectedCountryList2[_i];
      })["Color"]);
      line_svg.append("text").attr("class", "legends").attr("x", w + 70).attr("y", 36 + 25 * _i).text(selectedCountryList2[_i]).style("font-size", "12px").attr("alignment-baseline", "middle").attr("fill", "grey");
    };

    for (var _i = 0; _i < selectedCountryList2.length; _i++) {
      _loop(_i);
    }
  ...
  function removeCountryElement() {
    ...
    this.parentNode.parentNode.removeChild(this.parentNode);
    var firstChildValue = this.parentNode.firstElementChild.textContent;
    var index = window.selectedCountryList.indexOf(firstChildValue);
...
},{"d3":"UzF0","../static/country_no_duplicate.csv":"O9ZS","../static/2015_2019_no_duplicate.csv":"LyQK"}]},{},["Focm"], null)
//# sourceMappingURL=https://cse442-20f.github.io/FP-Knock-On-The-Happiness-Door/src.2be77368.js.map